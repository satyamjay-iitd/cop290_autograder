...
///
/// GUIDELINES:-
/// 1. Solve top to bottom, i.e solve section 1 fist and section 4 last.
/// 2. Start by searching "todo", and replacing it with your solution.
/// 3. There are total X todos in 4 sections.
///    3.1 Section 1: a todos.
///    3.2 Section 2: b todos.
///    3.3 Section 3: b todos.
///    3.4 Section 3: b todos.
    ...
    fn eval(&self, ctx: &SymbolTable) -> Result<u32, EvalErr> {
        match self {
            ...
            Val::Var(sym) => ctx
                .get(sym)
                .copied()
                .ok_or(EvalErr::UninitializedVar(sym.clone())),
        }
    }
}
...
impl Statement {
    ...
    pub fn execute(self, ctx: &mut SymbolTable) -> Result<(), EvalErr> {
        let rhs_val = self.rhs.eval(ctx)?;
        ctx.insert(self.lhs, rhs_val);
        Ok(())
    }
}
...
impl Interpreter {
    ...
    pub fn step(&mut self) -> Result<&SymbolTable, InterpretErr> {
        match self.stmts.pop_front() {
            Some(s) => {
                s.execute(&mut self.st)
                    .map_err(|e| InterpretErr::EvalErr(e))?;
                Ok(&self.st)
            }
            None => Err(InterpretErr::ProgramAlreadyExitted),
        }
    }
...
// DONT READ THIS. IT IS USED FOR TESTING ONLY.
#[macro_export]
